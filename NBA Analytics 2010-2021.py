...
fig1 = px.scatter(df1, 
                    x=df1['Preds'], 
                    y=df1['W'],title="Step-Wise RMSE(2.618) R^2(0.954)",
                    height=500)

fig2 = px.scatter(df2, 
                    x=df2['Preds'], y=df2['W'],
                    title="Hand Picked Stats RMSE(4.359) R^2(0.874)", 
                    height=500)
#
...
'''
markdown_text4 = '''
### Most Similar NBA Players By Year and Position With Blocks Per Game
'''
markdown_text5 = '''
### Most Similar NBA Players By Year and Position With Assists Per Game
'''
markdown_text6 = '''
### Most Similar NBA Players By Year and Position With 3 Point Attempts Per Game
'''
...
app.layout = html.Div([
    ...
    dcc.Graph(id='graph4-with-slider'),
    dcc.Slider(
        id='year-slider4',
        min=df['Year'].min(),
        max=df['Year'].max(),
...
def update_graph(team_drop, team_drop2):
    ...
    fig.add_trace(
        go.Line(x=dff['Year'], 
                y=dff['W/L%']),row=1,col=1)

    fig.add_trace(
        go.Line(x=dfs['Year'], 
                y=dfs['W/L%']),row=1,col=2)

    fig.update_layout(height=600, 
                    width=1600, 
                    title_text="NBA Teams Win/Loss Comparison")     
...
def update_graph2(team_drop3, team_drop4):
    ...
    fig.add_trace(
        go.Line(x=dff['Year'], 
                y=dff['3PA']),row=1, col=1)

    fig.add_trace(
        go.Line(x=dfs['Year'], 
                y=dfs['3PA']),row=1, col=2)

    fig.update_layout(height=600, 
                    width=1600, 
                    title_text="NBA Teams 3 Point Attempts Comparison Over The Years")     

    return fig

@app.callback(
    Output('indicator-graphic4', 'figure'),
    Input('team_drop5', 'value'),
    Input('team_drop6', 'value'))

def update_graph2(team_drop5, team_drop6):
    dff = tdf[tdf['Team'] == team_drop5]
    dfs = tdf[tdf['Team'] == team_drop6]

    fig = make_subplots(rows=1, cols=2)

    fig.add_trace(
        go.Line(x=dff['Year'], 
                y=dff['2PA']),row=1, col=1)

    fig.add_trace(
        go.Line(x=dfs['Year'], 
                y=dfs['2PA']),row=1, col=2)

    fig.update_layout(height=600, width=1600, title_text="NBA Teams 2 Point Attempts Comparison Over The Years")     
...
def update_graph(Player_drop, Player_drop2):
    ...
    fig.add_trace(
        go.Line(x=dff['Year'], 
                y=dff['3P%']),row=1, col=1)

    fig.add_trace(
        go.Line(x=dfs['Year'], 
                y=dfs['3P%']),row=1, col=2)

    fig.update_layout(height=600, 
                    width=1600, 
                    title_text="NBA Player 3 Point Percentage Comparison") 
...
@app.callback(
    Output('graph4-with-slider', 'figure'),
